...
if(Server.player.hasTag('staffstatus', chatmsg.sender.nameTag)) {
    if(!args[0]) return Server.broadcast(`§¶§cLexus ► §c§lPlease specify the name of the player of whom you would like to see their inventory`, chatmsg.sender.nameTag );
	else if(!playerfound) return Server.broadcast( `§¶§cLexus ► §c§l${playername} is offline or does not exist`, chatmsg.sender.nameTag );
    else {
        // The framework for grabbing inventory was created by balloon from mcpe realm hub: https://discord.gg/P9Zd6wu97V
    ...
    Server.broadcast(`§¶§cLexus ► §c§lError 4: Only Staff can use this command`, chatmsg.sender.nameTag);
}
});