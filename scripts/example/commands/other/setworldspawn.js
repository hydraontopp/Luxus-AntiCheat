...
if(Server.player.hasTag('staffstatus', chatmsg.sender.nameTag)) {
    if(worldset.includes(args[0])) {    
        ...
        Server.broadcast(`§¶§cLexus ► §b§lWorld Spawn configured to §e${Server.player.getScore('Worldx', chatmsg.sender.nameTag)} ${Server.player.getScore('Worldy', chatmsg.sender.nameTag)} ${Server.player.getScore('Worldz', chatmsg.sender.nameTag)}§b! Players will be sent here after passing World Border`, chatmsg.sender.nameTag);
    }
        ...
        Server.broadcast(`§¶§cLexus ► §b§lCustom World Spawn has been set back to default`, chatmsg.sender.nameTag);
    }
    else {
